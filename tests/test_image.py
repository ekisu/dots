...
class TestImageLoader(unittest.TestCase):
    def test_inexistant_image(self):
        with self.assertRaises(RuntimeError):
            ImageLoader.from_path(asset_path_builder("inexistant_image.png"))

    def test_basic_jpg(self):
        image = ImageLoader.from_path(
            asset_path_builder("2x1_black_white.jpg")
        )
    ...
    def test_basic_png(self):
        image = ImageLoader.from_path(
            asset_path_builder("2x1_black_white.png")
        )
        expected_grayscale = np.array([
            [0, 255]
        ], dtype=np.uint8)
    ...
    def test_transparency_mask(self):
        image = ImageLoader.from_path(
            asset_path_builder("2x1_opaque_transparent.png")
        )
        ...
        self.assertTrue(
            (transparency_mask == expected_transparency_mask).all()
        )
